...
class Framework(object):
    ...
    def train(self, model_pattern):
        ...
        for epoch in range(self.config.n_epoch):
            ...
            if (epoch + 1) % self.config.test_epoch == 0:
                ...
                precision, recall, f1_score = self.test(dev_data_loader, model, h_bar=self.config.bar, t_bar=self.config.bar)
                model.train()
        ...
        self.logging("best epoch: {}, best f1: {}, precision: {}, recall: {}, total time: {:5.2f}s".format(best_epoch, best_f1_score, best_precision, best_recall, time.time() - init_time))

    def test(self, test_data_loader, model, output=False, h_bar=0.01, t_bar=0.01):
        if output:
    ...
    def testall(self, model_pattern):
        ...
        precision, recall, f1_score = self.test(test_data_loader, model, output=True, h_bar=self.config.bar, t_bar=self.config.bar)
